...
refresh_compile_commands(
    name = "refresh_compile_commands",
    exclude_headers = "all" # Omit all headers from compile_commands.json
    ...
)
```

This defaults to None - all headers required by your sources, including those from external workspaces and elsewhere on your system (/usr/include, etc.)
will be included.

If you still want to include headers, but just yours, you can use `"external"`.
This will include headers from your workspace, as well as any generated by your build (in bazel-out/ for example)

```
refresh_compile_commands(
    name = "refresh_compile_commands",
    exclude_headers = "external" # Generate entries for headers in your workspace, but no others
    ...
)
```
...
By default, sources from external workspaces are included in compile_commands.json.  If you don't need these, you can
use `exclude_external_sources`:

```
refresh_compile_commands(
    name = "refresh_compile_commands",
    exclude_external_sources = True # Omit entries form any sources in external workspaces
    ...
)
```
